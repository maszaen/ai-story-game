  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden"
      ...
      <div className="absolute bottom-0 left-0 w-32 h-32 opacity-30"
        style={{ borderBottom: '3px solid #c9a84c', borderLeft: '3px solid #c9a84c', margin: '20px' }} />
      <div className="absolute bottom-0 right-0 w-32 h-32 opacity-30"
        style={{ borderBottom: '3px solid #c9a84c', borderRight: '3px solid #c9a84c', margin: '20px' }} />

      {/* Title Section */}
      <div className="text-center mb-10 z-10">
        <div className="mb-4">
          <span className="text-amber-700 text-lg tracking-[0.5em] uppercase font-medium" style={{ fontFamily: "'Cinzel', serif" }}>
            <IconSwords size={18} /> Infinite <IconSwords size={18} />
          </span>
        </div>
        <h1 className="text-6xl md:text-7xl font-bold mb-3 tracking-wider"
          style={{
        ...
        <div className="flex items-center justify-center gap-4 mb-4">
          ...
          <span className="text-amber-600 text-sm tracking-[0.3em] uppercase" style={{ fontFamily: "'Cinzel', serif" }}>
            Mesin Petualangan Tanpa Batas
          </span>
          <div className="h-px w-20 bg-gradient-to-l from-transparent to-amber-700" />
        </div>
        <p className="text-amber-900/70 text-sm italic max-w-md mx-auto" style={{ fontFamily: "'Crimson Text', serif" }}>
          "Setiap pilihan membentuk takdirmu. Setiap jalan menuntun ke petualangan baru."
        </p>
      </div>

      {/* API Key Warning */}
      {!hasApiKey() && (
        <div className="w-full max-w-md z-10 mb-6 stagger-child" style={{ animationDelay: '0.1s' }}>
          <div className="ornate-divider mx-4 mb-3">
            <span style={{ color: 'rgba(139,26,26,0.5)', fontSize: '10px' }}>◆</span>
          </div>
          <div className="parchment-bg rounded-lg p-5 text-center" style={{
            ...
            <p className="mb-3" style={{
              ...
              Masukkan Gemini API Key di Pengaturan untuk membuka gerbang petualangan.
            </p>
            <button
      ...
      <button
        onClick={onNewGame}
        disabled={!hasApiKey()}
        className={`group relative mb-4 z-10 ${!hasApiKey() ? 'opacity-40 cursor-not-allowed' : ''}`}
      >
        <div className="absolute inset-0 bg-amber-700/20 rounded blur-xl group-hover:bg-amber-700/40 transition-all duration-500" />
        <div className="relative px-12 py-4 rounded border-2 border-amber-700/60 hover:border-amber-500 transition-all duration-300"
          style={{
      ...
      <button
        onClick={onOpenSettings}
        className="group mb-10 z-10"
      >
        <div className="relative px-8 py-2.5 rounded border border-amber-800/40 hover:border-amber-600/60 transition-all duration-300"
          style={{
            background: 'linear-gradient(180deg, rgba(20,14,8,0.7) 0%, rgba(12,8,4,0.8) 100%)',
            fontFamily: "'Cinzel', serif",
          }}>
          <span className="text-amber-700/70 text-sm tracking-widest uppercase group-hover:text-amber-500 transition-colors flex items-center gap-2">
      ...
      {/* Saved Games */}
      <div className="w-full max-w-2xl z-10" style={{ minWidth: '420px' }}>
        <div className="flex items-center gap-4 mb-6">
          <div className="h-px flex-1 bg-gradient-to-r from-transparent to-amber-800/50" />
          <h2 className="text-amber-600 text-sm tracking-[0.3em] uppercase" style={{ fontFamily: "'Cinzel', serif" }}>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 border border-amber-900/30 rounded-lg"
            style={{ background: 'rgba(15,10,5,0.6)' }}>
            <p className="text-amber-800/60 italic" style={{ fontFamily: "'Crimson Text', serif" }}>
          ...
          <>
            <div className="space-y-3">
              {saves.slice(0, 2).map((save, index) => (
                <div
                  ...
                  style={{
                    background: 'linear-gradient(180deg, rgba(25,18,8,0.8) 0%, rgba(15,10,5,0.9) 100%)',
                ...
                >
                  <div className="flex items-stretch">
                    ...
                    <div className="w-28 h-20 flex-shrink-0 bg-amber-950/30 overflow-hidden">
                      {save.thumbnail ? (
                        <img src={save.thumbnail} alt="" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-amber-800/40 text-2xl"><IconShield size={24} /></div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 px-4 py-2 min-w-0">
                      <div className="flex items-center justify-between">
                        <h3 className="text-amber-500 font-semibold truncate" style={{ fontFamily: "'Cinzel', serif" }}>
                          {save.name}
                        </h3>
                      ...
                      <p className="text-amber-700/60 text-xs mt-1 truncate" style={{ fontFamily: "'Crimson Text', serif" }}>
                        {save.quests?.filter(q => !q.completed).map(q => q.text).join(' • ') || 'Petualangan menanti...'}
                      </p>
                      {save.isGameOver && (
                        <span className="text-xs mt-1 inline-flex items-center gap-1" style={{ color: '#8b1a1a', fontFamily: "'Cinzel', serif", fontSize: '0.6rem', letterSpacing: '0.1em' }}><IconSkull size={12} /> TAMAT</span>
                      )}
            ...
            {saves.length > 2 && (
              <div className="text-center mt-4 stagger-child" style={{ animationDelay: '0.5s' }}>
                <button
                  onClick={onOpenSavedGames}
                  className="group inline-flex items-center gap-2 px-6 py-2.5 rounded border border-amber-800/40 hover:border-amber-600/60 transition-all duration-300"
                  style={{
                    background: 'linear-gradient(180deg, rgba(20,14,8,0.7) 0%, rgba(12,8,4,0.8) 100%)',
                    fontFamily: "'Cinzel', serif",
                  }}
                >
                  <span className="text-amber-700/70 text-sm tracking-widest uppercase group-hover:text-amber-500 transition-colors">
      ...
      <div className="absolute bottom-6 text-center z-10">
        <p className="text-amber-900/40 text-xs tracking-wider" style={{ fontFamily: "'Cinzel', serif" }}>
