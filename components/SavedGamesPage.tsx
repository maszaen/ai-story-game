...
export const SavedGamesPage: React.FC<SavedGamesPageProps> = ({ onLoadGame, onBack }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4 relative overflow-hidden"
      ...
      {/* Back button */}
      <div className="w-full max-w-2xl z-10 mt-6 mb-4">
        <button
          onClick={onBack}
      ...
      <div className="text-center mb-8 z-10">
        ...
        <h1 className="text-3xl md:text-4xl font-bold tracking-wider mb-2"
          ...
          <IconScroll size={28} style={{ display: 'inline-block', marginRight: '8px', verticalAlign: 'middle' }} />
          Petualangan Tersimpan
        </h1>
        <p className="text-amber-800/60 text-sm italic max-w-md mx-auto" style={{ fontFamily: "'Crimson Text', serif" }}>
          {saves.length > 0 ? `${saves.length} petualangan tercatat dalam arsip` : 'Arsip masih kosong'}
        </p>
      </div>

      {/* Saves list */}
      <div className="w-full max-w-2xl z-10 pb-10" style={{ minWidth: '420px' }}>
        {loading ? (
          <div className="text-center py-16">
            <div className="inline-block w-10 h-10 border-2 border-amber-700/30 border-t-amber-500 rounded-full animate-spin" />
            <p className="mt-4 text-amber-700/50 text-sm" style={{ fontFamily: "'Cinzel', serif" }}>Memuat arsip...</p>
          </div>
        ) : saves.length === 0 ? (
          <div className="text-center py-16 border border-amber-900/30 rounded-lg"
            style={{ background: 'rgba(15,10,5,0.6)' }}>
            <IconShield size={48} style={{ display: 'inline-block', color: 'rgba(201,168,76,0.2)', marginBottom: '12px' }} />
